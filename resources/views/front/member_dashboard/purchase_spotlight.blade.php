    ...
    <form id="joinForm">
        @csrf
          <div class="mb-4 text-center">
            <label class="block text-sm font-medium text-gray-700">Business Spotlight</label>
            <p class="mt-1 text-lg font-semibold">$79.00</p>
        </div>
         <div class="mb-4">
            <label for="spotlight_month" class="block text-sm font-medium text-gray-700">Choose Spotlight Month</label>
            <select name="spotlight_month" id="spotlight_month" class="w-full mt-2 p-2 border rounded-lg">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (e) {
        ...
        if (!plainData.spotlight_month) {
            Swal.fire("Error", "Please select a spotlight month.", "error");
            return;
        }

        // Step 1: Create PaymentIntent
        let intentRes = await fetch("{{ route('memberdashboard.checkout-spotlight') }}", {
            method: "POST",
        ...
        if (paymentIntent.status === "succeeded") {
            ...
            if (registerData.success) {
                Swal.fire("Success", "Your Business Spotlight is now active!", "success").then(() => {
